...
select sl.PROD_ID,avg(sl.AMOUNT_SOLD) from sh.sales sl group by sl.PROD_ID;

-- 3. Find the maximum and minimum sale amount for each product.  
select sl.PROD_ID,MAX(sl.AMOUNT_SOLD),MIN(sl.AMOUNT_SOLD) from sh.sales sl group by sl.PROD_ID;

-- 4. Find the total revenue generated by each product.
select sl.prod_id,sum(sl.QUANTITY_SOLD*sl.AMOUNT_SOLD) AS TOTAL_REVENUE from sh.sales sl group by sl.prod_id;

-- 5. Count the number of sales transactions for each customer.  
select sl.CUST_ID,count(sl.QUANTITY_SOLD) AS TOTALSALES from sh.sales sl group by sl.CUST_ID;

-- 6. Find the average sale amount per customer.  
select sl.CUST_ID,AVG(sl.QUANTITY_SOLD*sl.AMOUNT_SOLD) AS AVGCUSTAMT from sh.sales sl group by sl.CUST_ID;

-- 7. Find the highest purchase amount for each customer.  
select sl.CUST_ID,MAX(sl.QUANTITY_SOLD*sl.AMOUNT_SOLD) AS MAXPURCHASEAMT from sh.sales sl group by sl.CUST_ID;

-- 8. Find the lowest purchase amount for each customer.  
select sl.CUST_ID,MIN(sl.QUANTITY_SOLD*sl.AMOUNT_SOLD) AS MINPURCHASEAMT from sh.sales sl group by sl.CUST_ID;
